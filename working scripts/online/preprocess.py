...
class Preprocessor:
    def __init__(self, artifact_threshold=3000.0):
        ...
        # Compute indices in the 64-channel stream that match the 58 training electrodes
    ...
    def process(self, window: np.ndarray) -> np.ndarray:
        ...
        #print(f"[Preprocessor] Before subsetting: channels = {original_channels}")

        # Subset & reorder
        subset_window = window[:, self.subset_indices]

        # Print after subsetting
        new_channels = subset_window.shape[1]
        #print(f"[Preprocessor] After subsetting: channels = {new_channels}")
